...
const rules = [
    ...
    "DOMAIN-SUFFIX,uestc.edu.cn,DIRECT",
    "DOMAIN-SUFFIX,m-team.cc,DIRECT",
    "DOMAIN-SUFFIX,labuladong.online,默认节点",
    "DOMAIN-SUFFIX,nowcoder.com,DIRECT", 
    "DOMAIN-SUFFIX,115,DIRECT", 
...
const regionDefinitions = [
   {
  name: '🌏 低倍率(HK/JP/SG)',
  regex: /^(?=.*(?:港|香港|🇭🇰|HK|[Hh]ong\s*[Kk]ong|新加坡|狮城|🇸🇬|SG|[Ss]ingapore))(?=.*(?:0\.[1-9]\s*[xX倍]?|低倍|下载|优化))(?!.*(?:家宽|住宅|Residential|Home|家用|宽带|Broadband|IPLC|IEPL|专线|HKT|CMI|9929|AS4837))(?!.*(?:[1-9]\d*(?:\.\d+)?\s*(?:[xX×✕✖⨉倍倍率])|[xX×✕✖⨉]\s*[1-9]\d*(?:\.\d+)?))/u,
...
//https://github.com/666OS/rules/blob/release/mihomo/domain/Emby.txt
  emby_ip: {
    ...ruleProviderCommonIpcidr,
    ...ruleProviderFormatMrs,
...
function main(config) {
  ...
  // 新增：排除专线节点（关键字可根据你的命名调整）
  const dedicatedLineFilter = /专线|premium|directline|private/i;

  // 排除匹配到的节点
  if (config.proxies && Array.isArray(config.proxies)) {
    config.proxies = config.proxies.filter( (proxy) => !excludeFilter.test(proxy.name) && !dedicatedLineFilter.test(proxy.name), 
    );
  }
  ...
  for (const proxy of proxies) {
    ...
    // HK/JP/SG 低倍率优先
  const lowAsia = regionGroups['🌏 低倍率(HK/JP/SG)'];
  
  if (
    regionDefinitionsEnable['🌏 低倍率(HK/JP/SG)'] &&
    regionDefinitions.find(r => r.name === '🌏 低倍率(HK/JP/SG)').regex.test(name)
  ) {
    lowAsia.proxies.push(name);
    continue;
  }
  ...
  regionDefinitions.forEach((r) => {
    ...
    if (groupData.proxies.length > 0) {
      generatedRegionGroups.push({
        ...
        //tolerance: 100,
        icon: r.icon,
        proxies: groupData.proxies.length ? groupData.proxies : ['DIRECT'],
        interval: 14400,
        tolerance: 50,
      });
    }
  ...
  groupProxies = [
    ...aiPriority,
    '默认节点',
    ...regionGroupNames.filter(
      p => !aiPriority.includes(p) && !aiExclude.includes(p)
    )
  ];
} else if (svc.key === 'youtube' || svc.key === 'emby') {
  ...
  // 组装最终结果
    // --- 策略组引用安全检查 ---
  // 防止某个策略组引用了没有实际生成的策略组，导致 Mihomo 报：
  // "proxy group xxx: proxy 'xxx' not found"
  //
  // 例如：
  // 订阅中没有日本节点
  // → 🇯🇵 日本地区组不会生成
  ...
  const allGeneratedGroupNames = new Set([
    ...generatedRegionGroups.map((group) => group.name),
    ...functionalGroups.map((group) => group.name),
  ]);

  // DIRECT / REJECT / REJECT-DROP / PASS 等内置策略也属于合法引用
  const builtinProxies = new Set([
    'DIRECT',
    'REJECT',
    'REJECT-DROP',
    'PASS',
  ...
  for (const group of functionalGroups) {
    ...
    group.proxies = group.proxies.filter((proxyName) => {
      if (builtinProxies.has(proxyName)) {
        return true;
      }

      // 已生成的策略组
      if (allGeneratedGroupNames.has(proxyName)) {
        return true;
      }

      // 原始代理节点
      if (proxies.some((proxy) => proxy.name === proxyName)) {
        return true;
      }
